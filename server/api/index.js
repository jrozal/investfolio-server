...
async function getMarketIndexData(dateData) {
  try {
    ...
    const urls = symbols.map((symbol) => {
      if (symbol !== "BTC/USD") {
        return axios.get(
          `${finnhubURL}/quote?symbol=${symbol}&token=${finnhubApiKey}`
        );
      } else {
    ...
    const chartUrls = symbols.map((symbol) => {
      if (symbol !== "BTC/USD") {
        return axios.get(
          `${finnhubURL}/stock/candle?symbol=${symbol}&resolution=60&from=${yesterday}&to=${currentTime}&token=${finnhubApiKey}`
        );
      } else {
        // crypto markets open 24/7/365
        const yesterday = currentTime - 86400; // 24 hrs ago (in seconds);
        return axios.get(
          `${finnhubURL}/crypto/candle?symbol=BINANCE:BTCUSDT&resolution=60&from=${yesterday}&to=${currentTime}&token=${finnhubApiKey}`
        );
      }
    });
    ...
    console.error("getMarketIndexData error", err);
  }
}
